...
export class MusicDatabase {
  ...
  async _setUserField(field, value) {
    ...
    await syncManager.updateUserJSON(user.uid, field, value);
    return true;
  }
  ...
  _minifyItem(type, item) {
    ...
    if (type === "track") {
      return {
        ...
        artist:
          item.artist ||
          (item.artists && item.artists.length > 0 ? item.artists[0] : null) ||
          null,
  ...
  _dispatchPlaylistSync(action, playlist) {
    window.dispatchEvent(
      new CustomEvent("sync-playlist-change", { detail: { action, playlist } })
    );
  }
  ...
  async createPlaylist(name, tracks = [], cover = "", description = "") {
    ...
    const playlist = {
      ...
      name,
      tracks: (tracks || []).map((t) =>
        this._minifyItem("track", { ...t, addedAt: now() })
      ),
      cover,
      description,
      createdAt: ts,
